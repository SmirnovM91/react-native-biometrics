...
public class ReactNativeBiometrics extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void simplePrompt(String title, String fingerprint_message, String fingerprint_cancel, String fingerprint_hint, String fingerprint_recognized, String fingerprint_not_recognized, Promise promise) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ReactNativeBiometricsDialog dialog = new ReactNativeBiometricsDialog();
                dialog.init(title, null, getSimplePromptCallback(promise));
                dialog.setTranslate(fingerprint_message, fingerprint_cancel, fingerprint_hint, fingerprint_recognized, fingerprint_not_recognized);
                Activity activity = getCurrentActivity();
                dialog.show(activity.getFragmentManager(), "fingerprint_dialog");
            } else {
